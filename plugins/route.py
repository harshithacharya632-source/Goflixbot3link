...
async def root_route_handler(request):
    return web.json_response({"status": "Bot Online"})
...
async def media_streamer(request: web.Request):
    try:
        ...
        if match:
            ...
            id = int(re.search(r"(\d+)(?:\/\S+)?", path).group(1))
            secure_hash = request.rel_url.query.get("hash")
        return await _stream_file(request, id, secure_hash)
    except InvalidHash as e:
        raise web.HTTPForbidden(text=e.message)
    except FIleNotFound as e:
        raise web.HTTPNotFound(text=e.message)
...
async def _stream_file(request: web.Request, id: int, secure_hash: str):
    """
    Stream Telegram file in chunks with support for multi-client load balancing.
    """
    ...
    # Use cached ByteStreamer object
    ...
    file_name = file_id.file_name or f"{secrets.token_hex(4)}.unknown"
